...
proc server_loop_1 :: | Colors, LogConsole => =
    | colors, console => -> do
        hcase colors of
            ColorPutRed -> do
                get fruit on colors
...
proc server_close_then_loop1 :: [Char] | TopBot, Colors, LogConsole => =
...
defn
    proc server_loop :: | Colors, Colors, LogConsole => =
        | colors1, colors2, console => -> do
            -- Version A: does not compile
            race
                colors1 -> hcase colors1 of
                    ColorPutRed -> server_echo( False | colors1, colors2, console =>)
                    ColorClose -> server_close_then_loop1("Done 1" | colors1, colors2, console =>)
                colors2 -> hcase colors2 of
                    ColorPutRed -> server_echo( True | colors2, colors1, console =>)
    ...
    proc server_echo :: Bool | Put([Char] | RedFruits), Colors, LogConsole => =
        swtch | fruits, colors, console => -> do
            get fruit on fruits
            ...
            hcase fruits of
                RedFruit -> do
                    put fruit on fruits
                    case swtch of
                        False -> server_loop( | fruits, colors, console => )
                        True -> server_loop( | colors, fruits, console => )

proc server :: | Colors (*) Colors, LogConsole => =
    | _2_colors, console => -> do
        split _2_colors into colors1, colors2
        server_loop( | colors1, colors2, console => )
