...
A process with a sequential input of type $A$, an input channel of type $X$, and an output channel of type $Y$ is defined in CAMPL as in Figure \ref{..}. It can be equivalently expressed as a circuit diagram, show in Figure{..} and the sequent calculus shown in Figure {..}. The black dot at the lower end of the input wire in the circuit diagram indicates the wire carries a sequential input. 
...
The following program plugs process \texttt{p} with process \texttt{q} via channel \texttt{y}. Process \texttt{p} is connected to the output end of channel \texttt{y} while process \texttt{q} is connected to the input end of channel \texttt{y}.
...
If a process is connected to a channel of type $A (*) B$ at its output end, then it can {\sf fork} into two processes with two distinct channels $A$ and $B$ of output polarity. Dually, if a process is connected to a channel of type $A (+) B$ at its  input end, then it can {\sf fork} into two processes with two distinct channels $A$ and $B$ of input polarity. {\sf fork} is a non-blocking call. Accordingly, sequent rules for for fork are shown in Figure \ref{Fig: sequent rules fork}:
\begin{figure}[h]
...
Figures \ref{Fig: fork1} show a process {\sf p} connected to a channel of type $Y1 (*) Y2$ at its output end. Using the compound channel, the process forks into a process {\sf p} and a process {\sf q}. Similarly, Figure \ref{Fig: fork2} is an example of process forking at the input end.
...
\begin{minipage}{.5\textwidth}
	...
	\end{verbatim}
  	\vspace{-2em}
  \captionof{figure}{Process forking at the output end}
  \label{Fig: fork1}
\end{minipage}%
\begin{minipage}{.5\textwidth}
  \centering
	...
	\end{verbatim}  
	\vspace{-2em}
  \captionof{figure}{Process forking at the input end}
  \label{Fig: fork2}
\end{minipage}
\end{figure}

If a process is connected to a channel of type $A (*) B$ at its input end, then it can {\sf split} the bundle into two distinct channels $A$ and $B$ of input polarity to be used by the process for further computations. Dually, if a process is connected to a channel of type $A (+) B$ at its  output end, then it can {\sf split} the bundle into two two distinct channels $A$ and $B$ of output polarity. {\sf split} is a blocking call --- that is a process calling split will be blocked unless a {\sf fork} occurs at the other end of the same channel.
...
Figure \ref{Fig: split1} shows a process {\sf q} splitting an input channel and calling a process $p$. Dually, Figure \ref{Fig: split2} shows a process splitting on the output channel.
...
The semantics of message passing in CaMPL is given by linear actegory.. The operator $\circ$ in linear actegory corresponds to the {\sf Put} and the operator $\bullet$ corresponds to the {\sf Get} in CaMPL.Given channel of type $A \circ X$ or \texttt{Put(A | X)} means that process connected to its output end may send a message of type $A$ to the process connected to the input end. 

Figure \ref{Fig: sequent rules for Put} shows the sequent rules for \texttt{Put}. 
\begin{figure}[h]
	\[ \infer[\circ_R]{A,B ~|~ X \dashvv A' \circ Y}{A \dashv A' \quad B ~|~ X \dashvv Y} \]
	\[ \infer[\circ_L]{A ~|~ B \circ X \dashvv Y}{A,B ~|~ X \dashvv Y} \]
\vspace{-2em}
\caption{Sequent rules for Put}
\label{Fig: sequent rules for Put}
\end{figure}

Figure \ref{fig: process Put} shows a sample communication via \texttt{Put} type. Process \texttt{p} is connected to the output end while process \texttt{q} is connected to the input end of channel $x$ of type \texttt{Put([Char]|X)}. Process \texttt{p} send a string using \texttt{put} which is received by process \texttt{q} via \texttt{get}. 
