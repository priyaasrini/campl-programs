...
The {\tt Get} and {\tt Put} types are used to send and receive messages.
The operations that are performed on each polarity of these types are explained in Section \ref{subsec:put-get}. 
The {\tt Neg} type swaps the polarity of a channel so that two channels of same polarity can be identified as the same channel. See Section \ref{sec:neg} for more information. 
The {\tt TopBot} type is used to end communication over a channel, see Section \ref{sec:close}. 
The ${\tt (+)}$ and ${\tt (*)}$ types indicate a bundle of channels. 
The operations that are performed on each polarity of these types are explained in Section \ref{sec:split-fork}. 
