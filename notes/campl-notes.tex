...
\author{
    Robin Cockett\thanks{University of Calgary} \and Daniel Hashimoto\thanks{Universidade Federal do Rio de Janeiro} \and Melika Norouzbeygi\thanks{University of Calgary} \and Priyaa Varshinee Srinivasan\thanks{Tallinn University of Technology}}
...
\[ \infer[cut]{A, B ~|~ X \vvdash Z}{A ~|~ X \vvdash Y \quad \quad B ~|~ Y \vvdash Z } \]
...
\begin{itemize}
	\item 'send' a message of type {\tt A} over a right polarity (output) channel, and
	\item 'receive' a message of {\tt A} over a left polarity (input) channel.
\end{itemize}

Dually, given a channel of type $B \bullet X$, a process may:
\begin{itemize}
	\item 'send' a message of type {\tt A} over a left polarity (input) channel, and
	\item 'receive' a message of {\tt A} over a right polarity (output) channel.
\end{itemize}
...
{\tt  Put(-|-)} type\footnote{ Disederata: We could like to race on handles or any blocking call. Current compiler support is for {\tt Put}.}. 
...
Any point during execution, the program has three processes -- a process {\tt MemAccess} which reads and writes to the memory, a process {\tt MemWait}, and a Memory cell process. Process {\tt MemAccess} is connected to the Memory via an output channel. Process {\tt MemWait} is connected to Memory cell via {\tt MemAccess} on an output channel. 

It helps to view the processes {\tt A} and {\tt B} as alternatively running {\tt MemWait} and {\tt MemAccess}. Process {\tt A} starts with {\tt MemAccess} and process {\tt B} starts with {\tt MemWait}.

The channel of type {\tt Passer} acts similar to a Mutex variable. When the program starts running, {\tt MemWait} passes {\tt Passer} handle to {\tt MemAccess} and blocks (on {\tt split}).

On receiving the handle from {\tt MemWait}, {\tt MemAccess} accesses the memory cell -- by reading the existing value and the write a new value. 


Then, it forks on the input channel Passer {\tt Passer} producing two input channels of {\tt MemCell} and {\tt Neg(Passer)} types. It equates the {\tt MemChannel} type channel to the existing channel to the Memory cell. It passes the {\tt Neg(Passer)} channel to the {\tt MemWait} after negation using {\tt plug} command. 

At this point, {\tt MemWait} is unblocked (by the {\tt MemAccess} of Process 1) and invokes {\tt MemAccess} to access the memory cell.

During the program execution, the topology alternates between (a) and (b) depending on Process 1 or Process 2 accessing the memory cell respectively.
\[ \includegraphics[scale=0.1]{figs/MemCell.jpeg} \]

The protocol {\tt Passer} is used communication between the {\tt MemWait} and {\tt MemAccess} processes. 


The protocol {\tt Passer} has an input channel type {\tt M} and an output channel type {\tt S}. The handle {\tt Passer} type constructs from {\tt S} a compound {\tt (+)} channel. The protocol {\tt MemCh} is used communication between Process 1 and Memory cell, and allows reading, writing and closing a memory cell.
...
The process {\tt memCell} takes an input channel, reads and writes to it.
...
Initially, the main process creates three processes {\tt memAccess}, {\tt memWait}, and a memory cell with an initial value. Note that, {\tt memAccess} is connected to the memory cell.
\begin{center}
\begin{varwidth}{\linewidth}
	\begin{verbatim}
		proc run =
...
Process {\tt memWait} activates the {\tt passer} channel and splits the output channel, hence blocking itself. Once it resumes (due to a {\tt fork}), it {\tt plug}s to process {\tt memAccess} via the memory channel and negated passer obtained from split. The passer needs to be negated to be converted to an input channel to pass to {\tt memAccess}.
\begin{center}
\begin{varwidth}{\linewidth}
	\begin{verbatim}
	    proc memWait :: 
	    [Char] |  => Passer( | MemCh([Char] | )), StringTerminal =
...
Process {\tt memAccess} on receiving the {\tt Passer} handle on {\tt passer}, does read and write of the memory (lines ?? -- ??). Once memory access is complete, it forks on the input channel {\tt passer}. It equates new memory channel to the existing one {\tt mem}, and passes the negated version of the new passer as an output channel to {\tt memWait} using {\tt plug}.

\begin{center}
\begin{varwidth}{\linewidth}
	\begin{verbatim}
	    proc memAccess :: 
	    [Char] | Passer( | MemCh([Char] | ) ) => MemCh([Char]| ), StringTerminal =
