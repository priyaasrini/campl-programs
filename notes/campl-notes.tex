...
A complementary pair in CAMPL is a pair of process commands which in-unison is used by two connected processes for communication. In a complementary pair, one of the commands is of blocking nature until the other command is unblocking. For example, the  {\tt get}  command is used to receive a message, hence it blocks, until another process performs a corresponding {\tt put} command which is used to send a message.
