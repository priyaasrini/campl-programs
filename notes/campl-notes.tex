...
Complementary operator occur in pairs where one of the operators blocks the calling process until another process executes its complementary operator. For example, when a receiving process performs a {\tt get} on a input channel, it blocks until the sending process performs a {\tt put} on the opposite (output) end of the channel.
...
In all the examples, the processes interact with the outside world using (one of the two) special channels: a protocol called {\tt StringTerminal} and a coprotocol {\tt Console}. Console channel enables reading and writing to the terminal from which the campl program is run. Each StringTerminal channel, on the other hand, is connected to an Alacritty terminal, and allows reading and writing to the terminal. These special (co)protocols are available in the Prelude file.
...
\subsection{GetUntil}		   
This example extends the "Hello World" example by reading input from the console and printing it to the console. In this example, the process named {\tt getSomethingUntil} reads an input string from the console recursively and prints it (on the console). When the user enters the string {\tt q}, the process halts.
			    ...
			    | console => -> do 
			        ...
			        case isExit(input) of 
			            ...
			            False -> do 
			                ...
			                getSomethingUntil( | console => )   --recursive call
		\end{verbatim}
	\end{varwidth}
\end{center}
The following code snippet belongs to the helper function:
\begin{center}
	\begin{varwidth}{\linewidth}
		\begin{verbatim}
...
In this example, we expand the previous one to have two user-defined processes -- a client and a server. A client reads input string from the terminal, and sends it to the server. The server on listens to the client and on receiving an input string, it prints it to the console. When the user enters {\tt q}, the client and the server halt.

The processes use \texttt{Echo} protocol for communication. It provides a handle \texttt{EchoSend} of Put-Get type which will be used to send-receive or receive-send a message depending on the polarity of channel ends. The handle \texttt{EchoClosed} is used to close a channel of \texttt{Echo} type. 
...
\textcolor{blue}{[TOPOLOGY PICTURE]}

The client process is connected to an output channel of type \texttt{Echo}. It first reads a string (provided by the user) from the terminal. If the input string is not {\tt q}, then the client will place \texttt{EchoSend} handle on the channel followed by a \texttt{put} to send the string. Otherwise, it will place the \texttt{EchoClose} handle on the channel and halts.
...
\begin{center}
	...
	\end{varwidth}
\end{center}	
The client and the server processes are invoked as follows:
\begin{center}
	\begin{varwidth}{\linewidth}
		\begin{verbatim}
			proc run = 
...
This example demonstrates two clients racing to send a message to an echo server. The server prints the messages in the order they were sent. The server halts after printing both the messages. The clients halt as soon as they send. 
\[ \includegraphics[scale=0.1]{figs/non-det.jpeg} \]

The {\tt run} process initiates two clients and a server:
...
The server races the client processes. When one of the client wins, it calls a helper process to receive message from the winning client.
...
The helper processor invoked by the server, receives message from the winning client, outputs it on the console, and receives message from the losing client, outputs on the console and halts.

\begin{center}
\begin{varwidth}{\linewidth}
	\begin{verbatim}
		proc receive_message_and_continue :: 
			| Put([Char] | TopBot), Put([Char] | TopBot), Console => = 
...
Each client is connected to its own StringTerminal. When a client receives input from user in its terminal, it sends it to the server.
...
When the program is run, it opens two Alacrity windows (one for each client) waiting for input from user. 
\[ \includegraphics[scale=0.45]{figs/race-output.PNG}\]

\subsection{Mutual Exclusive access of shared memory}
...
In this extended example, we demonstrate how a process can be encoded as a message and sent to another process, which can then decodes the message and runs the process it has received.

In this example, we will encode {\tt proc helloworld} from Section \ref{sec:Hello World} into a sequential message. The client process sends this message to the server, which then runs the {\tt Hello World}.
