...
The following table summarizes the operations on the concurrent types.

 \begin{table}[!h]
\centering
	\begin{tabular}{|l|l|}
	\hline
	\textsf{id} & equates two channels \\ \hline
	\textsf{neg} & negates a channel \\ \hline
	\textsf{plug} & connects two processes by a channel \\ \hline
	\textsf{get} & gets a value on a channel \\ \hline
	\textsf{put} & puts a value on a channel \\ \hline
	\textsf{hput} & puts a handle on a channel \\ \hline
	\textsf{hcase} & cases on the handles obtained on channel \\ \hline
	\textsf{split} & splits a channel into two channels \\ \hline
	\textsf{fork} & forks two new processes \\ \hline
	\textsf{close} & closes a channel \\ \hline
	\textsf{halt} & closes a channel. Usually the last channel is halted. \\ \hline
	\end{tabular}
