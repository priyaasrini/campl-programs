...
In CaMPL, processes send and receive messages from one another via channels. CaMPL provides built-in channel types from which one may build custom channel types. Given a channel of certain type, the commands that can be performed on that channel depends on the polarity, see Section \ref{sec:concurrent-operators}.

The following are the built-in channel types provided by CaMPL \cite[Section 3.3.2]{Prashanth-thesis}.

\begin{tabular}{r l}
{\tt Get (A|Ch)} & channel type  takes as arguments a sequential type {\tt A} and a channel of type {\tt Ch}. \\
{\tt Put (A|Ch)} & channel type takes as arguments a sequential type {\tt A} and a channel of type {\tt Ch}. \\ 
{\tt Neg (Ch)} & channel type takes as arguments a channel of type {\tt P}. \\ 
{\tt TopBot} & which is acronym for as arguments {\tt Top} and {\tt Bottom} does not take arguments. \\
