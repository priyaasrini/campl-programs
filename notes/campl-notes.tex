...
In CaMPL, processes talk to each other, that is, send and receive messages, via channels. CaMPL provides built-in channel types from which one may build custom channel types. Given a channel of certain type, the commands that can be performed on that channel depends on the polarity, see Section \ref{sec:concurrent-operators}.
...
Using CaMPL, a programmer may design communication over a channel using {\em protocols} and {\em coprotocols}. Protocols and coprotocols are concurrent analogue of inductive and co-inductive data types in functional programming languages. Instances of protocols and coprotocols are called channels.
...
In the above code, {\tt EchoSimple} is a protocol that takes in an output conncurrent type {\tt S}, an input sequential type A, but no input concurrent types. {\tt EchoSend} and {\tt EchoClose} are called {\bf protocol handles} and are analogous to session types. 

\[ \xymatrix{ A~\blackman \ar@{-}@<-5pt>[rrr]^{(+) ~~ {\tt ch: EchoSimple} ~~ (-)} & & & \blackman~B } \]

Consider two processes $A$ and $B$ connected by a channel {\tt ch} of type {\tt EchoSimple} protocols. $(+)$ refers to output polarity and $(-)$ refers to input polarity. A channel of protocol type {\tt EchoSimple} is {\em activated} when the process in the output end places a handle on the channel using {\tt hput} command, see Section \ref{subsec:pairs}. Once a channel activated, the channel may be used for communication in accordance to the handle  defintion. For example, process $A$ can send a message to process $B$ after it places the {\tt EchoSend} handle. Once the send is complete, the channel goes back to the concurrent type $S$. The {\tt EchoClose} handle takes in a concurrent type {\tt S} and converts it into {\tt TopBot} type (thereby disallowing any further communication).
...
\item Lines {\tt 3.} cause {\tt processA} and {\tt process B} to be connected by  channel {\tt y} and to be run in parallel. The channel {\tt y} is of output polarity for {\tt processA} and input polarity for {\tt processB}. Note that, the sequential argument of type {\tt A} is duplicated, and passed to both {\tt ProcessA} in line {\tt 3} and {\tt ProcessB} in line {\tt 4}. Such duplication is not allowed for channels (since channels are linear resources).
\end{itemize}
