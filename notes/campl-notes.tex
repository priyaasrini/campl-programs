...
In all the examples, the processes interact with the outside world using (one of the two) special channels: a protocol called {\tt StringTerminal} and a coprotocol {\tt Console}. Console channel refers to the console from which the campl program is run, and enables reading and writing to the console. Each StringTerminal channel, on the other hand, is connected to an Alacritty terminal, and allows reading and writing to the terminal. These special (co)protocols are available in the Prelude file, and are hard-coded in the compiler.
